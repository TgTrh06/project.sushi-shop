...
export class MongooseCategoryRepository implements CategoryRepository {
  ...
  async update(id: string, input: UpdateCategoryInput & { slug?: string }) { const doc = await this.model.findByIdAndUpdate(id, { $set: input }, { new: true, runValidators: true }).lean(); return doc ? this.map(doc) : null; }
