...
export class MongooseReservationRepository implements ReservationRepository {
  ...
  async transition(id: string, expected: ReservationStatus, next: ReservationStatus, context: TransactionContext) { const doc = await this.model.findOneAndUpdate({ _id: id, status: expected }, { $set: { status: next } }, { new: true, runValidators: true, session: sessionOf(context) }).lean(); return doc ? this.map(doc) : null; }
  async updateStatus(id: string, status: ReservationStatus, context: TransactionContext) { const doc = await this.model.findByIdAndUpdate(id, { $set: { status } }, { new: true, runValidators: true, session: sessionOf(context) }).lean(); return doc ? this.map(doc) : null; }
